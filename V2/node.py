...
class Node:
    def __init__(self,state,parent: Optional[Node] = None,action_taken: Optional[int] = None,prior: float = 0.0,):
        self.state        = state
        self.parent       = parent
        self.action_taken = action_taken          # int (gate id) oppure None (root)
    ...
    def ucb_score(self, c_puct: float) -> float:
        ...
        # Se il nodo è root, evita divisione per 0 in sqrt:
    ...
    def backpropagate(self, value: float) -> None:
        """
        Propaga il valore V(s) (già dal punto di vista del giocatore root) verso l’alto.
        value dovrebbe stare in un range stabile (es. [-1,1]).
        """
        self.visit_count += 1
    ...
    def q_value(self) -> float:
        """Media dei valori back-propagati (Q)."""
        return 0.0 if self.visit_count == 0 else self.value_sum / self.visit_count

    def is_leaf(self) -> bool:
        """Un nodo è foglia se non è ancora stato espanso (children vuoto)."""
