...
class PruneModel(nn.Module):
    ...
    def fwd_train(self, states, scalars, pi, returns, lambda_H: float = 0.02):
        ...
        # Entropia (positiva): H = -Σ p log p
        probs    = log_probs.exp()
        entropy  = -(probs * log_probs).sum(dim=-1).mean()
        ent_loss = entropy.detach()  # per logging

        # ---- value loss (quantile regression)
        y = returns.view(B)  # oppure returns.squeeze(-1)
        val_loss = _QuantileValueProj.quantile_loss(quant, y)

        #TODO per loggin in caso posso anche mettere value.detach()
    ...
    def fwd_infer(self, states, scalars, legal_mask=None, top_k=None):
        B = states.size(0)
        scalars = scalars.view(B, 1)
        logits, value = self.forward(states, scalars)  # logits: (B,N)
        
        if legal_mask is not None:
            logits = logits.masked_fill(legal_mask == 0, -1e9)
        probs = F.softmax(logits, dim=-1)               # (B,N)

        # --- Top-k (se vuoi restringere lo spazio dei figli) ---
        ...
        action_idx = torch.argmax(probs, dim=-1)  # (B,)
